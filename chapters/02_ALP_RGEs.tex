...
The CP-violating interactions of an axion-like particle (ALP) with SM fields below the
...
renormalization group equations (RGEs), which can be schematically written as
\begin{equation}
...
$\mathcal{O}_i(x)$, and $\gamma_{i \leftarrow j}$ is the anomalous-dimension matrix regulating
the energy evolution of $c_i$ at the desired perturbative order.
...
The on-shell method used throughout this chapter is developed in
Chapter~\ref{ch:RGEmethod}; here we only recall the master formulae that are applied
repeatedly in the following, and then discuss in detail the two independent ways of performing
the phase-space integrals, namely via angular variables~\cite{Zwiebel:2011bx} and through the
use of Stokes' theorem~\cite{Mastrolia:2009dr}.

For an effective Lagrangian $\mathcal L_{\text{EFT}} = \sum_i c_i\, \mathcal O_i/\Lambda^{[\mathcal O_i]-4}$,
the form factor of the operator $\mathcal O_i$ is defined as
\begin{equation}
    F_i(\vec n;q)= \frac{1}{\Lambda^{[\mathcal O_i]-4}}\mel{\vec n}{\mathcal O_i(q)}{0}\,,
    \label{eq:ALPFF}
\end{equation}
namely as the matrix element between an outgoing on-shell state
$\bra{\vec n}=\bra*{1^{h_1},\dotsc,n^{h_n}}$ and an operator $\mathcal O_i$ that injects an
additional off-shell momentum $q$.
At one-loop order and in the case of linear operator mixing, the anomalous dimensions follow
from
\begin{equation}
...
\end{equation}
evaluated at the Gaussian fixed point $(*)$, where all the Wilson coefficients $c_i$ vanish,
while for the non-linear mixing among operators of different dimensions one
...
\end{equation}
with $j,k\neq i$.
The right-hand sides of Eqs.~\eqref{eq:ALPmasterformula} and \eqref{eq:ALPmastergammaijk}
correspond to a sum over all one-loop two-particle unitarity cuts,
\begin{align}
\label{eq:ALPMF1Loop}
    (\mathcal M F_j)(1,\dotsc,n) =
   \sum_{k=2}^n \sum_{\{x,y\}}\int \text{dLIPS}_2
   %\nonumber\\&\times
   \sum_{h_1,h_2} F_j(x^{h_1},y^{h_2},k + 1,\dotsc,n)\, \mathcal M(1,\dotsc,k;x^{h_1},y^{h_2})\, ,
\end{align}
where $\mathcal M(\vec n; \vec m)=\mel{\vec n}{\mathcal M}{\vec m}$ and $\text{dLIPS}_2$ is the
two-particle Lorentz-invariant phase-space measure.
Furthermore, by making use of the Higgs low-energy
theorem~\cite{Ellis:1975ap,Shifman:1979eb}, leading mass effects can be included while still
working in a massless formalism~\cite{Bresciani:2023jsu}.
In practice, whenever an amplitude requires $N$ fermion mass insertions not to vanish, we
consider an equivalent amplitude entailing $N$ extra massless Higgs fields, where
\begin{equation}
N = 4-[\mathcal{O}_{i}] +\sum^n_{k=1} ([\mathcal{O}_{j_k}]-4)\geq 0
\label{eq:ALPsuperficial_div}
\end{equation}
corresponds to the superficial degree of divergence associated with the loop diagram under
consideration~\cite{Bresciani:2023jsu}.
The anomalous dimension $\gamma_{i\leftarrow j_1, \dotsc, j_n}$ is then obtained by
renormalizing the operator $(h/v)^N\mathcal{O}_i/N!$ instead of $\mathcal{O}_i$, while for
$N < 0$ it vanishes.

Since the method relies on dimensional regularization, it is sensitive only to the difference
between ultraviolet and infrared divergences.
Therefore, in order to disentangle the renormalization group equations for the ultraviolet
divergent part, the infrared contribution must be computed independently; this is done in
Appendix~\ref{app:ALP_IR}.

The Lorentz-invariant phase-space measure appearing in Eq.~\eqref{eq:ALPMF1Loop} can be
...
ultraviolet singularities of massless one-loop amplitudes, together with collinear infrared
...
ultraviolet singularities carried by the two-point integrals, neglecting the higher-point
...
Within the coefficient of the two-point integrals, both ultraviolet and infrared collinear
divergences are combined.
Therefore, in order to isolate the genuine ultraviolet anomalous dimension, one has to compute
...
Hereafter, we detail the computation of the ultraviolet anomalous dimensions relevant to ALP
...
between the ultraviolet and infrared anomalous dimensions, the knowledge of the latter is
...
one can use BCFW recurrence relations~\cite{Britto:2004ap,Britto:2005fq}.} which, in general,
reads
...
the QED fine-structure constant at one-loop level.
...
\caption{Diagrammatic formula for computing $\gamma_{\text{E}_{ij}\leftarrow S_{kl},\tilde\gamma}$.}
...
\begin{align}
    (\mathcal M F_\gamma)|_{*,\mathcal C_\gamma \neq 0} (1^-_{f_i},2^+_{\bar f_j}, 3^-_{f'_k},4^+_{\bar f'_l}) &= \sum_h \int \text{dLIPS}_2 \,\bigg[ \mathcal M|_{*,\mathcal C_\gamma \neq 0}(3^-_{f'_k},4^+_{\bar f'_l}; x_{\gamma}^h , y_\phi) F_{\gamma}|_*(1^-_{f_i},2^+_{\bar f_j},x_{\gamma}^h , y_\phi) \nonumber \\
    &\quad +
...
In the following, we summarize our results for the renormalization group equations of the
...
\begin{tikzpicture}
	...
	\end{feynman} 
	\end{tikzpicture}
\caption{Unitarity cuts that contribute to the RGEs of the class $X^2H^2$. Dashed lines with an
...
\begin{tikzpicture}
	...
	\end{feynman} 
	\end{tikzpicture}
\caption{Unitarity cuts that contribute to the RGEs of the classes $H^6$ and $H^4 D^2$. Dashed
...
\begin{tikzpicture}
	...
	\end{feynman} 
	\end{tikzpicture}
\caption{Unitarity cuts that contribute to the RGEs of the classes $\psi^2 XH$ and
...
\begin{tikzpicture}
	...
	\end{feynman} 
	\end{tikzpicture}
\caption{Unitarity cuts that contribute to the RGEs of the class $\psi^2 H^3$. Dashed lines
...
\qquad \qquad \qquad 
	...
	\end{feynman} 
	\end{tikzpicture}
\caption{Unitarity cuts that contribute to the RGEs of four-fermion operators. The left diagram
...
Q_{q \ell}^{(1)prst} &= (\overline{\ell}_p \gamma^\mu \ell_r)(\overline{q}_s \gamma_\mu q_t)\,,  &Q_{q \ell}^{(3)prst} &= (\overline{\ell}_p \gamma^\mu \sigma^I \ell_r)(\overline{q}_s \gamma_\mu \sigma^I q_t)\,,
\end{align}
...
\begin{tikzpicture}
	...
	\end{feynman}
	\end{tikzpicture}
\caption{Feynman diagrams that do not contribute to the renormalization of the vertex
$\phi G G$ because they are either identically zero or because they give rise to no ultraviolet
divergences.}
...
associated with a pair of photons, whose long-distance dynamics is clearly dictated by their
kinetic term.
...
\begin{tikzpicture}
	...
	\end{feynman}
	\end{tikzpicture}
\caption{Triangle and bubble diagrams contributing to the renormalization of the three-gluon
...
They are completely constrained, up to an overall factor given by the coupling constant, by
...
\end{align}
Locality implies $[g_\mathsf{H}],[g_\mathsf{A}]<1$; therefore we can infer that the holomorphic
(anti-holomorphic) configuration is the consistent one if $h_1+h_2+h_3<0$ ($h_1+h_2+h_3>0$).
The case where $h_1+h_2+h_3=0$ is trivial, as it can only correspond to a cubic scalar
...
The on-shell method is not directly sensitive to the ultraviolet anomalous dimension associated
...
gauge theory, as
\begin{equation}
    \gamma_{\text{IR}} (\{p_i\}, \mu) = \frac{g^2}{4 \pi^2} \sum_{i<j} T^a_{ik} T^a_{kj} \log \frac{\mu}{-s_{ij}} + \sum_i \gamma_i^{\text{coll.}}\,,
\end{equation}
...
gauge-invariant operator with a vanishing ultraviolet anomalous dimension and allowing for
two-particle interactions.
In this respect, a natural candidate is given by the energy-momentum tensor $T_{\mu\nu}$.
Since the energy-momentum tensor has to be conserved also at the quantum level, its ultraviolet
...
\subsection{\texorpdfstring{$\phi FF$ and $\phi F\widetilde F$ Operators}{phi FF and phi F Ftilde Operators}}
...
\subsection{\texorpdfstring{$\phi GG$ and $\phi G\widetilde G$ Operators}{phi GG and phi G Gtilde Operators}}
