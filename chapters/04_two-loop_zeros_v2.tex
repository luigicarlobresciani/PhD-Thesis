...
\begin{figure}[t]
    ...
    \caption[Unitarity cuts of two-loop form factors]{Unitarity cuts relevant for the extraction of anomalous dimensions from two-loop form factors. Higher-dimensional operator insertions are indicated by red blobs. The blobs with a hole denote one-loop form factors or amplitudes.}
    \label{fig:cuts}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{equation}
A geometric representation of these selection rules is presented in Figure~\ref{fig:cone}.

\begin{figure}[tbhp]
    \centering
    \includegraphics[scale=0.9]{images/cone.pdf}
    \caption[Structure of $L$-loop operator mixing]{Operator mixing in the plane of total helicity $h$ and length $\ell$,
relative to a source $\mathcal{O}_j$. Each cell is a site
$(\Delta h,\Delta\ell)=(h_i-h_j,\ell_i-\ell_j)$ with $\Delta h+\Delta\ell$
even, and its color and number give the lowest loop order at which the
scheme-independent selection rules allow $\mathcal{O}_j$ to mix into
$\mathcal{O}_i$. The operator cone, $|\Delta h|\le|\Delta\ell|$ or,
equivalently, $\Delta w\,\Delta\overline w\ge0$ with $w=\ell-h$ and $\overline w=\ell+h$,
contains exactly the operators reached at the first order allowed by the
length selection rule~\cite{Bern:2019wie}, $\max(1,1-\Delta\ell)$. Outside the
cone, the helicity selection rule forbids mixing at that
order (see \Eq{eq:gen}), except up to the dashed line when
non-holomorphic Yukawa couplings are present. The numbers are lower bounds:
an entry may still vanish, for instance when no unitarity cut exists.}
    \label{fig:cone}
\end{figure}
...
In a generic scheme, such as the modified minimal-subtraction scheme, the entries in the region of \Eq{eq:dl2} are generally
...
\begin{figure}[t]
    ...
    \caption[Factorization channels of the one-loop form factor $F^{(1)}_j$]{Factorization channels of the one-loop form factor $F^{(1)}_j$ with $w<w_j-2$ or $\wb < \wb_j - 2$. (a)~One-loop amplitude glued to a tree form factor, which is forbidden by the weight
