...
\subsubsection*{Effective Field Theories and On-Shell Methods}
...
To address the points above and overcome these challenges, in this thesis we employed, as a common methodology, on-shell
amplitude techniques, a modern reformulation of perturbation theory that replaces the traditional
...
one, where they appear as apparently accidental cancellations. Instead, in the on-shell framework they follow
from selection rules based on helicity, operator length, and angular momentum. 
...
\begin{itemize}
    ...
    results in the literature. These include the dimension-five and -six operators generated once the ALP is
    integrated out, such as the three-gluon Weinberg operator and the (chromo-)electric and (chromo-)magnetic
    ...
    carry no independent two-loop information, being entirely fixed by one-loop data. Moreover, for general operators
...
theorem, which set a target for the Large Hadron Collider. 
% However, their standard formulation applies only to $2\to2$
...
% for spin-two and higher-spin particles, relevant for gravitational EFTs.
However, their standard formulation applies only to $2\to2$ processes, whereas the partial waves of $2\to N$ processes can grow faster with the energy and are expected to dominate at future facilities. Furthermore, it is impractical for theories with spin-two and higher-spin particles, which are relevant for gravitational EFTs.
Moreover, positivity
bounds restrict the signs of combinations of Wilson
coefficients and, therefore, complement unitarity bounds, which restrict their magnitudes. 
...
\begin{itemize}
    ...
    derived the bounds on the quartic interactions of photons and gravitons, and analyzed their interplay with positivity bounds.
...
% coefficients. Anomalous dimensions describe how they evolve with the energy, while unitarity
...
Second, on-shell methods have recently been extended to the matching of ultraviolet (UV) models onto their EFTs,
...
Third, positivity bounds and spinning sum rules are part of a broader web of constraints that has emerged from recent advances, including the
