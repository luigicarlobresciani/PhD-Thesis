\chapter{Unitarity and Positivity Bounds in the Axion-Like Particle Effective Field Theory}
...
\section{The Axion-Like Particle Effective Field Theory}

ALP interactions with SM fermions and gauge bosons 
...
\input{tables/ALP_dim5-6}
\input{tables/ALP_dim7}
\input{tables/ALP_dim8}

\section{Unitarity Bounds}
\label{unitarity_bounds}
...
However, this is not necessarily the case, as was shown for instance in \cite{Bertuzzo:2023slg} for the production of a Higgs-ALP pair at a lepton collider: the impact of the dimension-seven operators $\partial_\mu \phi \,D^\mu \overline \ell H e$ and $\partial_\mu \phi \,\overline \ell H D_\mu e$ on the total cross section was there shown to be significantly larger than the one due to the dimension-five operator $\phi \,\overline{\ell} H e$ in a wide region of the parameter space.

Motivated by these considerations, we stress the possible relevance of placing unitarity constraints on higher-dimensional ALP interactions, which constitutes the subject of this section.
...
\begin{figure}[h!tb]
    ...
    \caption{Summary of the marginalized partial-wave unitarity bounds on the Wilson coefficients associated with the dimension-five and -six ALP operators.
    }
...
\begin{figure}[h!tb]
    ...
    \caption{Summary of the marginalized partial-wave unitarity bounds on the Wilson coefficients associated with the dimension-seven ALP operators.
    }
    \label{fig:d7summary}
\end{figure}
...
\begin{figure}[h!tb]
    ...
    \caption{Summary of the marginalized partial-wave unitarity bounds on the Wilson coefficients associated with the dimension-eight ALP operators.}
    \label{fig:d8summary}
\end{figure}
...
\end{equation} 
%
where the $SU(2)_L$ invariance is recovered for $g_{\nu_l} = \overline g_{ll} - g_{ll}$. The implications of the above interactions are better understood by integrating the Lagrangian by parts~\cite{Altmannshofer:2022ckw}:
%
\begin{align}
...
\begin{figure}[h!tb]
    ...
    \caption{Parameter space allowed by partial-wave unitarity bounds (Eq.~\eqref{eq:d8-uni_phi2H2}, blue) and positivity bounds (Eq.~\eqref{eq:d8-pos_phi2H2}, red) associated with the operators $\op{8}{\phi^2 H^2}{(1)}$ and $\op{8}{\phi^2 H^2}{(2)}$. The intersection area constitutes $1/4$ of the area of the blue region.}
    \label{fig:d8phi2H2}
\end{figure}
...
\begin{figure}[h!tb]
    ...
    \caption{Parameter space allowed by the partial-wave unitarity bounds 
...
\begin{figure}[thb]
    ...
    \caption{Parameter space excluded by the partial-wave unitarity bounds in Eq.~\eqref{eq:PWUB_d5_phiX2} for the dimension-five ALP couplings to photons ($\coef{5}{\phi F^2}{} = c^2_{\rm W} \, \coef{5}{\phi B^2}{} + s^2_{\rm W} \,\coef{5}{\phi W^2}{}$) and $Z$ bosons ($\coef{5}{\phi Z^2}{} = s^2_{\rm W} \, \coef{5}{\phi B^2}{} + c^2_{\rm W}\,\coef{5}{\phi W^2}{}$), in the cases where $\coef{5}{\phi F^2}{} \coef{5}{\phi Z^2}{} < 0$ (left panel) and $\coef{5}{\phi F^2}{} \coef{5}{\phi Z^2}{} > 0$ (right panel). In both cases the coupling to gluons have been fixed to $\abs*{\coef{5}{\phi G^2}{}} = 0.225$~TeV$^{-1}$.
...
\subsection{\texorpdfstring{Dimension-Seven Couplings in $l^+ l^- \to \phi h$}{Dimension-Seven Couplings in l+ l- -> phi h}}
...
is $\sigma = \sigma^{(5)} + \sigma^{(7)}$, where
%
\begin{equation}
...
%
\begin{equation}
    \sigma^{(7)} \le \frac{5\pi}{3s}\,,\qquad\qquad
...
\begin{figure}[thb]
    ...
    \caption{Cross section for the process $l^+ l^- \to \phi h$, with $l=e,\mu$, in the high-energy limit. The relevant Wilson coefficients are parameterized as $\coef{5}{\phi \ell e H}{ll} = i \, y_l/f$ and $\coef{7}{\phi \ell e H}{(1)ll} = \coef{7}{\phi \ell e H}{(2)ll} = 1/f^3$, where $f$ can be interpreted as the ALP decay constant.}
...
\begin{figure}[thb]
    ...
    \caption{Theoretical and experimental bounds on the coupling $g_{ee}$ of a weak-violating ALP~\cite{Altmannshofer:2022ckw}. 
