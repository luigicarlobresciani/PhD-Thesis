...
\begin{tikzpicture}[
    ...
    \node[eqn] at (5.0,2.6) {\textbf{Running}\quad
        ...
        $\displaystyle \widehat c_a(\mu)\simeq \widehat c_a(\Lambda)-\sum_b\widehat \gamma_{ab}\,\widehat c_b(\Lambda)\log\frac{\Lambda}{\mu}+\dots$};
...
\subsubsection*{Anomalous Dimensions and the Structure of Operator Mixing}
...
truncation of Eq.~\eqref{eq:intro_Leff} questionable there. Therefore, an interpretation of such
...
intricate rapidly, since the number of Feynman diagrams grows rapidly with the number of loops and
...
\begin{itemize}
    ...
    bounds to processes with arbitrary multiplicity and spin. We apply it to the anomalous quartic interactions
    ...
    \item Chapter~\ref{ch:SMEFT_bounds}, based on Ref.~\cite{Bresciani:2026acy} and on a companion
    article, performs a coupled-channel analysis of the dimension-six SMEFT over all helicity, gauge,
