...
\end{tikzpicture}
\caption[Analytic structure of the forward amplitude $\mathcal M(s,0)$ and the dispersive contour]{Analytic structure of the crossing-symmetric forward amplitude $\mathcal M(s,0)$ in the complex $s$ plane. The
...
\begin{equation}\label{eq:superposition}
    \mathcal M(s,t) = \mathcal M(u,v\to u,v)
    = \sum_{i,j,k,l} u_i^{*}v_j^{*}u_kv_l\,\mathcal M(k,l\to i,j)\,.
\end{equation}
The
...
\begin{equation}
    \mathbb H_i\,\varepsilon_{+}^{\dot\alpha\alpha}(p_i,\xi) = +\varepsilon_{+}^{\dot\alpha\alpha}(p_i,\xi)\,,
    \qquad
    \mathbb H_i\,\varepsilon_{-}^{\dot\alpha\alpha}(p_i,\xi) = -\,\varepsilon_{-}^{\dot\alpha\alpha}(p_i,\xi)\,.
\end{equation}
...
\end{itemize}
In both configurations, the four-momenta need to be complexified, so that $\lambda_i$ and $\tildee \lambda_i$ can be treated as independent variables. For real momenta the two configurations collapse onto each other, and any amplitude with a positive total bracket degree vanishes, while one with a negative degree diverges.
...
data of lower multiplicity whenever the deformation can be arranged to fall off in that limit. This is the content of the Britto--Cachazo--Feng--Witten recursion
