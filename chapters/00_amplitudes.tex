...
\end{align}
with $i,j=1,\dotsc,n$, where we have introduced the north-west-to-south-east $\searrow$
(south-west-to-north-east $\nearrow$) contraction rule for the undotted (dotted) Weyl indices.
...
\end{tikzpicture}
\caption{Analytic structure of the forward amplitude $\mathcal M(s,0)$ in the complex $s$ plane. The
...
the Froissart bound guarantees for $n\ge2$.
...
\paragraph{ALP/dilaton coupling to gauge bosons.}
...
$\Tr T^aT^b =\delta^{ab}$, and the sum runs over the permutations of the labels $2,\dotsc,n$. The coefficients $A_n$ are the \textit{color-ordered} or \textit{partial amplitudes}. They are gauge
