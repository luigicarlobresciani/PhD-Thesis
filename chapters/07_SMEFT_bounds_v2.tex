...
\textit{This chapter is based on Ref.~\cite{Bresciani:2026acy} and on a companion paper:}\\[0.5em]
L.~C.~Bresciani, P.~Paradisi and A.~Sainaghi,\\[0pt]
``Unitarity bounds and sum rules in the SMEFT,''\\[0pt]
[\href{https://doi.org/10.48550/arXiv.2603.03423}{arXiv:2603.03423} [hep-ph]];\\[0.5em]
L.~C.~Bresciani, P.~Paradisi and A.~Sainaghi,\\[0pt]
``Unitarity bounds and spinning sum rules in the flavorful SMEFT,''\\[0pt]
(to be published).
...
As discussed in Chapter~\ref{ch:generalizedUB}, a key theoretical consistency requirement for any EFT
...
which conditions they may fail. In Section~\ref{sec:SMEFT_multiflavor} we relax the single-flavor
assumption and present the bounds on four-fermion operators under the MFV and $U(2)^5$ flavor
...
ultraviolet completions onto the semileptonic operators, and Appendix~\ref{app:SMEFT_math} gathers
the mathematical tools used in the derivation of the bounds.
...
To fully exploit the power of this method, we performed a complete coupled-channel analysis. In particular, to compute the amplitude of a process mediated by a given SMEFT operator, we considered all possible initial and final states in all allowed helicity, gauge, and flavor configurations. 
...
bound is imposed. For operators containing a single fermionic bilinear, the lowest allowed angular
...
operators of type $\psi^2\varphi^2D$ the outcome depends on the case at hand, since in the fully
coupled-channel analysis the color degrees of freedom can add up and make the $J=1$ bound stronger,
even though it would naively be expected to be more suppressed.

\input{tables/SMEFT_J_bilinear}

For four-fermion operators, the relevant angular momentum depends on the chirality of the fermions considered in the scattering processes. The channels contributing at $J=0$ are
\begin{gather}
...
dominates, but not always, which stresses once again that one cannot stop at the smallest allowed
angular momentum whenever several degrees of freedom contribute to the coupled-channel analysis.

\input{tables/SMEFT_J_fourfermion}
...
Altmannshofer:2025lun} have introduced dispersion relations that yield either spin-dependent sum
rules or constraints on the high-energy behavior of scattering amplitudes. Remarkably, the resulting
pattern of SMEFT inequalities can be used to infer characteristic features of the underlying UV
dynamics, indicating whether it is dominated by scalar or vector particles.
...
\begin{equation}\label{eq: sum rules with boundary}
    \begin{split}
        ...
        &=8\int_{s_0}^\infty \frac{\dd s}{s^2}\left\{-\Im a_{\overline{\alpha}\beta}^{0}(s)+\sum_{J=1}^\infty(2J+1)\Big[\Im a_{\alpha\beta}^{J}(s)-\Im a_{\overline{\alpha}\beta}^{J}(s)\Big]\right\}\,,
    \end{split}
\end{equation}
...
\end{equation}
If the boundary term vanishes, one can show that all partial waves $a_{\alpha\beta}^{J}$ and
$a_{\overline\alpha\beta}^{J}$ with $J\geq2$ vanish as well, together with $\Im
a_{\overline\alpha\beta}^{1}$. Under this condition, Eq.~\eqref{eq: sum rules with boundary} reduces
to~\cite{Remmen:2020uze}
\begin{equation}\label{eq: sum rules master equation}
    \lim_{s\rightarrow 0}\partial_s\mathcal{A}_{\overline \alpha\beta}(s,0) = -\lim_{s\rightarrow 0}\partial_s\mathcal{A}_{ \alpha\beta}(s,0) =8\int_{s_0}^\infty \frac{\dd s}{s^2}\left(\Im a_{\overline{\alpha}\beta}^{0}(s)-3\Im a_{\alpha\beta}^{1}(s)\right)\,.
\end{equation}
...
coefficients, unlike what happens for $\psi^4$ operators. For purely bosonic operators the boundary
...
\end{equation}
and requiring all their eigenvalues to be positive for every $\beta$. Without loss of generality, we
parameterize
\begin{equation}
...
every $\beta$ is therefore equivalent to requiring $f(x)\geq0$ for every $x\in[0,1]$.

This last step is what makes the procedure practicable. Scanning numerically over $\beta$ would give
only a sampled, and therefore inconclusive, version of the constraint, whereas the positivity of $f$
on the unit interval can be traded for a finite set of algebraic inequalities among the coefficients
$a,b,c,d,e$, which are themselves linear in the Wilson coefficients. Explicitly, $f(x)\geq0$ for
...
For vector dominance one follows the same steps, requiring instead all eigenvalues to be negative
definite.
...
\begin{itemize}
    \item They assume tree-level ultraviolet completions and can be violated at one
    loop~\cite{Azatov:2021ygj};
    ...
    particular for vector $t$-channel ultraviolet
    completions~\cite{Remmen:2020uze,Remmen:2022orj,Azatov:2021ygj}.
\end{itemize}
The last condition is the most problematic one, since tree-level ultraviolet completions of $\psi^4$
...
scalar- and vector-dominated regions does not exclude an ultraviolet completion but points to a more
...
tree-level ultraviolet completions of Ref.~\cite{deBlas:2017xtg}. All scalar completions satisfy the
...
symmetry group
\begin{equation}
    G_{\text{MFV}}= U(3)^5=U(3)_q\times U(3)_u\times U(3)_d\times U(3)_\ell\times U(3)_e\,,
\end{equation}
...
\end{equation}
The only interactions in the SM Lagrangian that explicitly break $G_{\text{MFV}}$ are the Yukawa
...
\end{equation}
Assuming that new physics also respects $G_{\text{MFV}}$, the number of independent coefficients in
four-fermion operators is strongly reduced, since MFV-violating terms can arise only through
...
symmetry is then
\begin{equation}
    G_{F}= U(2)^5=U(2)_q\times U(2)_u\times U(2)_d\times U(2)_\ell\times U(2)_e\,,
\end{equation}
where the fermion fields decompose as $\psi = (\psi^a,\psi_3)$, with $a=1,2$ and
...
coefficients involving fermionic fields in the multi-flavor case. As an example, under MFV the
operator $O_{\ell\ell}$ has only two independent coefficients,
\begin{equation}
    [C_{\ell\ell}]_{prst} = C_{\ell\ell}^{(1)}\delta_{pr}\delta_{st} + C_{\ell\ell}^{(2)}\delta_{pt}\delta_{rs}\,,
\end{equation}
so that the resulting theoretical bound can be displayed in a two-dimensional plot, as shown in
Figure~\ref{fig:Oll_mfv}.

Even under these hypotheses the coupled-channel analysis remains demanding. As an illustration,
Figure~\ref{fig:matrixplot} shows the structure of the scattering matrix associated with the
$q\overline q \to q \overline q$ processes in the three-flavor $U(2)^5$ scenario, whose analytic
eigenvalues were obtained with finite-field methods.
%
\begin{figure}[tb]
    \centering
...
In the following we list all the operator structures together with the marginalized bound on each
independent coefficient, for both the MFV and the $U(2)^5$ flavor hypotheses. Table~\ref{tab:summary}
collects the references to the tables of marginalized bounds and to the figures of the corresponding
allowed regions.

\input{tables/flavor_hyperlinks}

\input{tables/tables_flavor}

\input{images/flavor/figures_flavor}
...
We then derived the spinning sum rules for four-fermion operators in the Warsaw basis, keeping track
of the boundary terms of the dispersion relation, and gave an operative procedure to translate them
into inequalities on the Wilson coefficients in flavor space. We also delimited their regime of
validity, identifying three distinct mechanisms through which they can fail, and illustrated each of
them with explicit ultraviolet completions.
...
More broadly, the framework developed here establishes the first systematic, perturbativity-based
...
The function $f(x) = a + bx - \sqrt{c^2 + dx + ex^2}$ of Eq.~\eqref{eq:eigenvalue form} --- assumed
real on $[0,1]$ and with $a,b,c,d,e\in\mathbb{R}$ --- is such that $f(x) \ge 0$ for every
$x \in [0,1]$ if and only if the system of inequalities $\Delta(a,b,c,d,e)$ of Eq.~\eqref{eq:Delta}
is satisfied.
