...
The discovery of a scalar particle consistent with the Standard Model (SM) Higgs boson at the Large Hadron Collider (LHC) has firmly
...
electroweak scale. In this context, effective field theories (EFTs) provide a systematic framework to parameterize possible
...
Standard Model Effective Field Theory (SMEFT)~\cite{Buchmuller:1985jz,Grzadkowski:2010es,Brivio:2017vri,Isidori:2023pyp,Aebischer:2025qhh}.
...
which conditions they may fail. In Section~\ref{sec:SMEFT_multiflavor} we present the bounds on four-fermion operators under the minimal-flavor-violation (MFV) and $U(2)^5$ flavor
...
ultraviolet (UV) completions onto the semileptonic operators, Appendix~\ref{app:SMEFT_math} gathers the
...
Historically, the most widely adopted hypothesis is MFV, first introduced
...
We first analyze the unitarity bounds for purely bosonic operators. Figure~\ref{fig:histogram} shows the minimum energy scales $E_*$ at which the bounds in Table~\ref{tab:UniBounds} are violated, assuming Wilson coefficients are taken from the 95\% confidence level (CL) marginalized intervals of Ref.~\cite{Celada:2024mcf}, using linear (blue) and quadratic (orange) fits.
%
\begin{figure}[htbp]
    \centering
...
Moreover, Figure~\ref{fig:3plotsa} presents both the unitarity bounds and the experimental constraints from the Large Electron-Positron collider (LEP) and High-Luminosity LHC (HL-LHC) diboson data on the Wilson coefficients of the operators $O_{\varphi q}^{(1)}$ and $O_{\varphi q}^{(3)}$.
\begin{figure}[htbp]
    \centering
