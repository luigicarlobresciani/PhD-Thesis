...
considering all possible initial and final states in all allowed helicity, gauge, and flavor configurations.
...
defined as
\begin{equation}
    C_{\alpha\beta}^{(s,\infty)}=\Res_{s=\infty}\frac{\mathcal{A}_{\alpha\beta}(s,0)}{s^2}=-\lim_{\abs{s}\to \infty}\frac{1}{s}\mathcal{A}_{\alpha\beta}(s,0)\,.
\end{equation}
...
\begin{equation}\label{eq:eigenvalue form}
    f(x)=a + bx \pm \sqrt{c^2 + dx + ex^2}\,,\qquad x=\beta_1^2+\beta^2_2\,,
\end{equation}
...
$a,b,c,d,e$, which are themselves functions of the Wilson coefficients. Explicitly, $f(x)\geq0$ for
...
\begin{itemize}
    \item They assume tree-level UV completions and can be violated at loop level~\cite{Azatov:2021ygj};
...
tree-level UV completions of Ref.~\cite{deBlas:2017xtg}. All scalar completions satisfy the
...
\input{tables/tables_flavor_Olq}

\input{images/flavor/figures_flavor_Olq}
...
Figure~\ref{fig:matrixplot} shows the structure of the scattering matrix --- whose analytic
eigenvalues were obtained with finite-field methods~\cite{Chestnov:2025svg} --- associated with the
...
\input{images/flavor/figures_flavor_rest}
